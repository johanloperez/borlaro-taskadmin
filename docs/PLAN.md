...
**Ocupa el lugar del tablero, no flota encima.** La primera versión fue un modal centrado, con el argumento de que crear una tarea es una sub-acción de estar mirando el tablero y no conviene perder ese contexto. El argumento era correcto y la conclusión no: en una pantalla de 1280 px, un modal deja la mitad del ancho en fondo oscurecido, y una descripción con capturas y listas anidadas necesita ancho de verdad. El contexto no se pierde porque **al cerrar vuelve el Kanban intacto** —nunca se desmontó nada más que el área central—, que era lo que el modal protegía.

**Lo mismo para ver una tarea.** El panel lateral de 26 rem sigue siendo el modo por defecto, porque para mirar de reojo el estado de algo es lo correcto. Pero tiene un interruptor a pantalla completa, y la elección se recuerda: quien prefiere leer ancho lo prefiere para todas las tareas, y devolverlo al panel angosto en cada clic sería pelearle a su preferencia. En una columna de 26 rem una captura se achica hasta no leerse y cada línea se parte en tres.

**Las imágenes de la descripción se traen con la sesión.** Una captura adjunta se referencia como `![](/api/items/…/adjuntos/…)`, y los adjuntos no son públicos: un `<img src>` no puede mandar el header de autorización, así que esa imagen se vería rota. `MarkdownImage` la trae con `fetch` y la convierte en una URL de blob — el mismo camino que ya usaba el logo de la organización (§14). **Solo para lo que apunta a nuestra API**: pedir una imagen externa con nuestro token sería mandarle la credencial a un tercero.
