...
{
    private const string MutexName = "TaskAdmin.Desktop.SingleInstance";
    private const string RunKey = @"Software\Microsoft\Windows\CurrentVersion\Run";
    private const string RunValueName = "TaskAdmin";
    ...
    private async Task ArrancarAsync(string[] args)
    {
        ...
        _window.HideRequested += () => ShowBalloon("TaskAdmin sigue activo", "Lo encontrás en la bandeja del sistema.");
    ...
    private void ReportarFalla(Exception ex)
    {
        try
        {
            var carpeta = System.IO.Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "TaskAdmin");
        ...
        System.Windows.MessageBox.Show(
            $"TaskAdmin tuvo un problema y no pudo seguir.\n\n{ex.Message}\n\n" +
            "El detalle quedó en:\n%APPDATA%\\TaskAdmin\\errores.log",
            "TaskAdmin", MessageBoxButton.OK, MessageBoxImage.Warning);
    }
    ...
    private void OnConnectionStateChanged(bool connected)
    {
        Dispatcher.Invoke(() =>
        {
            ...
            {
                _tray.Text = connected ? "TaskAdmin — conectado" : "TaskAdmin — reconectando…";
            }
        });
    }
