...
public class AgentApprovalService(BorlaroTmsDbContext db, WorkItemService workItems)
{
    ...
    private async Task<string> ApplyFollowupAsync(AgentAction action, CancellationToken ct)
    {
        ...
        // El nivel lo propuso el agente al pedir la tarea: acababa de conversar sobre ese trabajo,
        // así que es quien mejor puede estimarlo. Si el modelo mandó cualquier cosa —o una versión
        // vieja del prompt no lo mandó— cae en Media, que es el punto medio y no arrastra al agente
        // hacia ninguno de los dos extremos. Quien aprueba lo ve y puede cambiarlo.
        var difficulty =
            Enum.TryParse<WorkItemDifficulty>(
                action.Arguments?.RootElement.TryGetProperty("difficulty", out var raw) == true
                    ? raw.GetString()
                    : null,
                ignoreCase: true,
                out var parsed)
                ? parsed
                : WorkItemDifficulty.Media;

        var item = await workItems.CreateAsync(
            projectKey,
            new CreateWorkItemRequest(
                title, description, null, WorkItemPriority.Normal, assigneeId, null, null, null, difficulty),
            ActorType.Agent, null, action.CheckInId, ct);
