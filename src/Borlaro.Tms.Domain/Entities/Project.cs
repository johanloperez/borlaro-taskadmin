...
public class Project : IOrganizationScoped
{
    ...
    /// <summary>Cómo llama este proyecto a los tres niveles de dificultad. Nulo = el nombre por
    /// defecto del idioma de quien mira.
    ///
    /// **Se renombra la etiqueta, nunca la escala.** Debajo siguen siendo `Baja`, `Media` y `Alta`
    /// —los mismos tres, en el mismo orden— y eso no es una limitación que se pueda levantar más
    /// adelante: es lo que hace que el agente sepa cuál extremo es cuál sin tener que inferirlo, y
    /// lo que permite comparar riesgo entre proyectos que no comparten vocabulario. Un equipo que
    /// necesite una taxonomía propia de verdad tiene los campos personalizados de tipo `Select`,
    /// que existen para eso y sobre los que el agente no razona.</summary>
    public string? DifficultyLabelLow { get; set; }
    public string? DifficultyLabelMedium { get; set; }
    public string? DifficultyLabelHigh { get; set; }
