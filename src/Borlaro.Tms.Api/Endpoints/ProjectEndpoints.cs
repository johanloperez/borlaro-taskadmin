...
public record NewProjectRequest(
    ...
    /// defecto. Se renombra la etiqueta, nunca la escala.</summary>
    DifficultyLabelsBody? DifficultyLabels = null);

/// <summary>Las tres etiquetas, siempre las tres juntas. Por separado se podría dejar una escala
/// mitad renombrada —«Simple / Media / Compleja»— que se lee como un error.</summary>
public record DifficultyLabelsBody(string? Low, string? Medium, string? High);
...
public record ProjectDetail(
    ...
    DifficultyLabelsBody DifficultyLabels,
...
public static class ProjectEndpoints
{
    public static IEndpointRouteBuilder MapProjectEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        templates.MapGet("/", async (BorlaroTmsDbContext db, CancellationToken ct) =>
        {
            ...
            try
            {
                var project = await service.CreateFromTemplateAsync(
                    new CreateProjectRequest(
                        ...
                        request.LeadIds,
                        principal.UserId(),
                        request.DifficultyLabels?.Low,
                        request.DifficultyLabels?.Medium,
                        request.DifficultyLabels?.High),
                    ct);
            ...
            CancellationToken ct) =>
        {
            ...
            return Results.Ok(new ProjectDetail(
                ...
                new DifficultyLabelsBody(
                    project.DifficultyLabelLow,
                    project.DifficultyLabelMedium,
                    project.DifficultyLabelHigh),
        ...
        /// <summary>Renombra los tres niveles de dificultad de este proyecto.
        ///
        /// Cambia cómo se llaman, no qué son: debajo siguen siendo Baja, Media y Alta, en ese
        /// orden. Es lo que le permite al agente saber cuál extremo es cuál sin inferirlo, y lo
        /// que deja comparar riesgo entre proyectos que no comparten vocabulario. Un equipo que
        /// necesite una taxonomía propia de verdad tiene los campos personalizados de tipo Select.
        ///
        /// Vaciar una etiqueta la devuelve al nombre por defecto, que se traduce al idioma de
        /// quien mira. Una etiqueta propia, no: la escribió alguien en un idioma y se muestra así.</summary>
        projects.MapPut("/{key}/dificultad", async (
            string key,
            DifficultyLabelsBody body,
            ClaimsPrincipal principal,
            ProjectAccess access,
            BorlaroTmsDbContext db,
            CancellationToken ct) =>
        {
            ...
            {
                return Results.Problem(
                    "Solo quien lidera el proyecto o un administrador pueden renombrar los niveles.",
                    statusCode: StatusCodes.Status403Forbidden);
            }
            ...
            static string? Limpiar(string? value) =>
                string.IsNullOrWhiteSpace(value) ? null : value.Trim();

            project.DifficultyLabelLow = Limpiar(body.Low);
            project.DifficultyLabelMedium = Limpiar(body.Medium);
            project.DifficultyLabelHigh = Limpiar(body.High);

            await db.SaveChangesAsync(ct);

            return Results.Ok(new DifficultyLabelsBody(
                project.DifficultyLabelLow, project.DifficultyLabelMedium, project.DifficultyLabelHigh));
        })
        .WithName("SetDifficultyLabels");
